...
run_leaks() {
  if [[ "$LEAK_AVAILABLE" -eq 0 ]]; then
    TEMPLEAK=""
    LEAKFLAG="not checked"
    return
  fi
  ($LEAK_CMD -atExit -- "$@" 1>a 2>b) & pid=$!
...
do
	...
	run_leaks $ROOT/push_swap $(cat $TRACE/error_files/test_case_$i.txt)
	printf "$TEMPLEAK\n"
	printf "\none argument\n\n"
	($ROOT/push_swap "$(cat $TRACE/error_files/test_case_$i.txt)" >stdout 2>stderr) & pid=$!
	(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
...
printf "test_case_16\n\n"
	...
	rm -rf stdout stderr
	run_leaks $ROOT/push_swap "123 1 2" "654 456 123"
	printf "$TEMPLEAK\n\n"
...
do
	...
	rm -rf stderr stdout
	run_leaks $ROOT/push_swap "$(cat $TRACE/identity_files/test_case_$i.txt)"
	printf "$TEMPLEAK\n\n"
done
...
do
	...
	run_leaks $ROOT/push_swap "$(cat $TRACE/size3/test_case_$i.txt)"
	printf "$TEMPLEAK\n\n"
done
...
do
	...
	rm -rf stdout stderr result error_result ok aa
	run_leaks $ROOT/push_swap "$(cat $TRACE/size5/test_case_$i.txt)"
	printf "$TEMPLEAK\n\n"
done
